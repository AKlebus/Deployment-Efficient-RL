...
# N.B time horizon == number of deployments
class StochasticDERLAgent: # Stochastic Deployment Efficient Reinforcement Learning Agent (Algorithm 2 from paper)
    def __init__(self, env, reward, roadmap, params = hp.default_stochastic):
        self.env = env
        self.reward = reward
        ...
        self.i_max = params['i_max'] # number of iterations
        self.beta = params['beta'] # bonus coefficient
        self.N = params['N'] # batch size
        self.H = params['H'] # time horizon
        self.epsilon_0 = 1 / (2*self.d *(self.N + 1)) # resolution for discretization
        self.v_min_squared = params['v_min_squared'] # magic coeficient. Use 1 for deterministic, 0.1 / 0.01 for stochastic according to Jiawei
    ...
    def plan_pi(self):
        ...
        for h in range(self.H, 0, -1):
            ...
            for s in range(self.state_size):
                for a in range(self.action_size):
                    phi = self._get_features(s, a)
                    Q[s, a] = np.minimum(np.dot(w, phi) + self.reward(h-1, s, a, self.roadmap, self.map_dimension) + u[s, a] , self.H)
    ...
    def train(self):
        ...
        for h in range(1, self.H+1):
            ...
            print(f"Explored layer {h}") if DEBUG else None
...
#     agent = StochasticDERLAgent(env = env, reward = FrozenLakeReward, roadmap=roadmap)  
